...
int main() {
    read(n, d);
    FOR(i, 1, n) read(a[i]);
    FOR(j, 1, n) FOR(i, 1, n) if (a[i] == j) vis[j] = 1;
    f[0][getfirst()] = 1;
    FOR(i, 1, n) {
        FOR(S, 0, (1 << (2 * d + 1)) - 1) {
            int now = (S >> 1) | (vis[i + d] << (2 * d));
            if (~a[i]) f[i][now] += f[i - 1][S];
            else FOR(j, -d, d) {
                if (i + j < 1 || i + j > n || now & (1 << (j + d))) continue;
                f[i][now | (1 << (j + d))] += f[i - 1][S];
            }
        }
    }
    print(f[n][(1 << (d + 1)) - 1]);
