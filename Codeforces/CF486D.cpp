...
const int maxn = 2005;
int n, d, head[maxn], to[maxn << 1], nxt[maxn << 1], cnte;
modint f[maxn], ans;

struct node {
    int val, id;
    il bool operator<(const node &rhs) {return val == rhs.val ? id < rhs.id : val < rhs.val;}
} a[maxn];
...
void dfs(int u, int fa, node rt) {
    f[u] = 1;
    for (int i = head[u]; i; i = nxt[i]) {
        int &v = to[i];
        if (v == fa || a[v] < rt || rt.val + d < a[v].val) continue;
        dfs(v, u, rt);
        f[u] *= (f[v] + 1);
    }
    return;
