...
const modint inv2 = qPow(2, mod - 2);
const int maxn = 1e6 + 5, N = 1e6;
modint k, fac[maxn], ifac[maxn], powinv2[maxn];
int n, m;

int main() {
    fac[0] = 1, powinv2[0] = 1;
    FOR(i, 1, N) fac[i] = i * fac[i - 1], powinv2[i] = inv2 * powinv2[i - 1];
    ...
    while (T--) {
        ...
        modint ans = 0;
        FOR(i, 1, m) ans += k * i * powinv2[n - i] * fac[n - i - 1] * ifac[m - i] * ifac[n - m - 1];
        print(ans);
    }
    return output(), 0;
}
