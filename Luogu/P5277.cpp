...
namespace poly {
#define int int
    const int G = 3, mod = MOD, maxn = 1 << 21 | 500, imgunit = 86583718;
    ...
    namespace QuadResidue {
        ...
        int cipolla(int n, int p = mod) {
            n %= p;
            if (p == 2) return n;
            if (poly::qPow(n, (p - 1) / 2) == p - 1) return -1;
            int a;
            while (1) {
                a = rand() % p;
                w = ((1ll * a * a % p - n) % p + p) % p;
                if (poly::qPow(w, (p - 1) / 2) == p - 1) break;
            }
            cmplx x = {a, 1};
            int res = qPow(x, (p + 1) / 2);
    ...
    void polySqrt(const Poly &A, Poly &B, int m) {
        if (m == 1) {
            B.push_back(QuadResidue::cipolla(A[0]));
            return;
        }
        polySqrt(A, B, (m + 1) >> 1);
        B.resize(m);
...
int main() {
    //print(QuadResidue::cipolla(1));putchar('\n');
    read(n); f.resize(lim = n);
    //lim <<= 2;
