...
int n, size[maxn << 1];
modint f[maxn << 1], fac[maxn], ifac[maxn];

modint lucas(int r, int k) {
    if (r < k) return 0;
    if (r <= min(n, mod - 1)) return fac[r] * ifac[k] * ifac[r - k];
    return lucas(r / mod, k / mod) * lucas(r % mod, k % mod);
}

int main() {
    read(n, mod);
    fac[0] = 1;
    FOR(i, 1, min(n, mod - 1)) fac[i] = fac[i - 1] * i;
    ifac[min(n, mod - 1)] = qPow(fac[min(n, mod - 1)], mod - 2);
    DEC(i, min(n, mod - 1) - 1, 0) ifac[i] = (i + 1) * ifac[i + 1];
    DEC(i, n, 1) {
        size[i] = 1 + size[i << 1] + size[i << 1 | 1];
        if (2 * i > n) f[i] = 1;
        else if (2 * i + 1 > n) f[i] = f[i << 1];
        else f[i] = lucas(size[i] - 1, size[i << 1]) * f[i << 1] * f[i << 1 | 1];
    }
    print(f[1]);
    return output(), 0;
}
