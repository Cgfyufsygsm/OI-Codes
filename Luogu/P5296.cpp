...
Poly exp(int x) {
    Poly ret(lim);
    int tmp = 1;
    FOR(i, 0, lim - 1) ret[i] = 1ll * tmp * ifac[i] % mod, tmp = 1ll * tmp * x % mod;
    return ret;
}
...
int Gauss(int n) {
    ...
    int rev = 0;
    FOR(i, 1, n) {
        if (!a[i][i][0]) FOR(j, i + 1, n) {
            if (a[j][i][0]) {
                swap(a[i], a[j]);
                rev ^= 1;
            }
        }
        auto inv = polyInv(a[i][i]);
        FOR(j, i + 1, n) {
            auto div = inv * a[j][i];
    ...
    return 1ll * (rev ? mod - ret[k] : ret[k]) * fac[k] % mod;
}

il void add(int u, int v, int w) {
    Poly f = exp(w);
    a[u][u] = a[u][u] + f, a[v][v] = a[v][v] + f;
    a[u][v] = a[u][v] - f, a[v][u] = a[v][u] - f;
    return;
}
...
int main() {
    cin >> n >> k;
    lim = k + 1, initInv();
    FOR(i, 1, n) FOR(j, 1, n) a[i][j].resize(lim);
    FOR(i, 1, n) FOR(j, 1, n) {
        cin >> g[i][j];
        if (i < j) add(i, j, g[i][j]);
    }
    cout << Gauss(n - 1) << endl;
    return 0;
}
