...
template<int r> struct Matrix {
    modint a[r][r];
    Matrix() {
        FOR(i, 0, r - 1) FOR(j, 0, r - 1) a[i][j] = 0;
    }
    void initI() {
        FOR(i, 0, r - 1)
            a[i][i] = 1;
    ...
    friend Matrix<r> qPow(Matrix<r> base, int exp) {
        Matrix<r> ret;
        ret.initI();
        for (; exp; exp >>= 1, base = base * base)
            if (exp & 1) ret = base * ret;
        return ret;
    }
};
