...
#define int int
    ...
    Poly polySin(const Poly &A) {
        return qPow(1ll * 2 * imgunit % mod) * (polyExp(imgunit * A) - polyExp((mod - imgunit) * A));
    }

    Poly polyCos(const Poly &A) {
        return qPow(2) * (polyExp(imgunit * A) + polyExp((mod - imgunit) * A));
    }

    Poly polytan(const Poly &A) {
        return polySin(A) * polyInv(polyCos(A));
    }
...
int main() {
    ...
    print(type ? polyAtan(f) : polyAsin(f));
    return output(), 0;
}