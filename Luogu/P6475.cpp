...
il modint binom(int n, int m) {return n < m ? 0 : fac[n] * ifac[n - m] * ifac[m];}
il modint f(int n, int m) {return binom(n + m - 1, n);}
...
int main() {
    ...
    modint ans = 0;
    if (x <= n && y > n)
        FOR(h, 1, m) ans += f(x - 1, h) * f(n - x, m - h + 1) * f(y - 1 - n, m - h + 1) * f(2 * n - y, h);
    else ans = f(n, m) * f(n - y + x, m);
    print(ans);
    return output(), 0;
}
}// namespace YangTY
