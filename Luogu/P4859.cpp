...
const int maxn = 2005;
int n, k, a[maxn], b[maxn], l[maxn];
modint dp[maxn][maxn], fac[maxn], invfac[maxn];

il modint binom(int n, int m) {return n < m ? 0 : fac[n] * invfac[m] * invfac[n - m];}

int main() {
    fac[0] = 1;
    FOR(i, 1, 2000) fac[i] = i * fac[i - 1];
    invfac[2000] = qPow(fac[2000], mod - 2);
    DEC(i, 1999, 0) invfac[i] = (i + 1) * invfac[i + 1];
    ...
    FOR(i, 1, n)
        FOR(j, 0, i)
            dp[i][j] = dp[i - 1][j] + (j ? (l[i] - j + 1) * dp[i - 1][j - 1] : 0);
    modint ans = 0;
    FOR(i, k, n) {
        modint tmp = binom(i, k) * fac[n - i] * dp[n][i];
        if ((i - k) & 1) ans -= tmp;
        else ans += tmp;
    }
    print(ans, '\n');
