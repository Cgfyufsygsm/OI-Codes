...
const int maxn = 40005, maxm = 2e6 + 5;
...
il int encode(int x, int y) {return (x - 1) * n + y;}
bool vis[maxn];
    ...
    FOR(x, 1, n) {
        FOR(y, 1, n) {
            ...
            FOR(k, 0, 7) {
                ...
                if (tx >= 1 && tx <= n && ty >= 1 && ty <= n && !vis[encode(tx, ty)])
                    add(encode(x, y), encode(tx, ty), 1), add(encode(tx, ty), encode(x, y), 0);
            }
        }
    }
    print(n * n - m - dinic());
